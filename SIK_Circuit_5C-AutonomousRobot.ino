...
int backupTime = 165;           //amount of time that the robot will back up when it senses an object
int turnTime =100;             //amount that the robot will turn once it has backed up
...
void loop()
{
  ...
  if(digitalRead(switchPin) == LOW){  //if the on switch is flipped
    ...
    if(distance < 4){                //if an object is detected
      //back up and turn
      Serial.print(" ");
      Serial.print("BACK!");
